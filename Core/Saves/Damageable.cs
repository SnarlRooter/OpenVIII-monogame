    ...
    public abstract class Damageable : IDamageable
    {
        ...
        public IReadOnlyDictionary<Kernel.AttackType, Func<int, Kernel.AttackFlags, int>> DamageActions
        {
            ...
            {   { Kernel.AttackType.PhysicalAttack, Damage_Physical_Attack_Action},
                ...
                int Damage_Physical_Attack_Action(int dmg, Kernel.AttackFlags flags)
                {
                    ...
                    var baseDamage = (strength * 16) + (weaponPower * 4);

                    // Apply target's VIT reduction
                    var targetVit = TotalStat(Kernel.Stat.VIT);
                    var vitReduction = (100 - targetVit) / 100f;
                    var finalDamage = (int)(baseDamage * vitReduction);
